...
def load_dms(dms_tensor):
    dms_fitness = torch.load(dms_tensor)
    if torch.isnan(dms_fitness).any():
        print(f"WARNING: NaNs in {dms_tensor}, replacing with 0")
    dms_fitness = torch.nan_to_num(dms_fitness, nan=0.0, posinf=0.0, neginf=0.0)
    return dms_fitness
...
#    n = len(seq)
#    dms_fitness = np.zeros((n,n,1), dtype=np.float32)
#    num = np.zeros(n)
#    den = np.zeros(n)
...
def main(args):
    ...
    for i in range(len(seq_list)):
        dms_file = tag_list[i][1][0] + '.pt'
        print(args.dms_dir)
        print(dms_file)
        dms_path = os.path.join(args.dms_dir, dms_file)
        print(dms_path)

        if not os.path.isfile(dms_path):
            print(f"Missing DMS file at path: {dms_path}")
            sys.exit(1)

        try:
            dms_fitness = load_dms(dms_path)#, seq_list[i][0])
            dms_list.append(dms_fitness)
        except Exception as e:
            print(f"[ERROR] Failed to load DMS for {tag_list[i][1][0]} at {dms_path}: {e}")
            traceback.print_exc()
            sys.exit(1)  # or `continue` if you want to skip this target
    ...
    for model, output_directory in model_generator:
        ...
        for (tag, tags), seqs, dms_fitness in sorted_targets:
            ...
            if feature_dict is None:
                ...
                feature_dicts[tag]["dms"] = dms_fitness
            ...
            print(tag)
            print("msa shape:", processed_feature_dict["msa_feat"].shape)
            print("msa_mask sum:", processed_feature_dict["msa_mask"].sum())
