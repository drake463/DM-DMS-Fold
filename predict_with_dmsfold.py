...
def load_dms(dms_csv,seq):
    ...
    expected_cols = ['seq_n','wt_res','mut_res','ddG']
    for col in expected_cols:
        if col not in df.columns:
            raise ValueError(f"Missing expected column '{col}' in {dms_csv}")
    ...
    print(dms_fitness)
    print(f"Loaded {np.sum(np.max(dms_fitness, axis=-1))} restraints...")
...
def main(args):
    ...
    for i in range(len(seq_list)):
        dms_file = tag_list[i][1][0] + '_dms.csv'
